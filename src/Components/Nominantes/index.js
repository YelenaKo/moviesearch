...
class Nominantes extends Component {
    constructor(props) {
        super(props);
        this.state = {  }
    }
    ...
    render() { 
        return (
            <div className = "nominantes-list">                
                {this.props.nominantList.map( element=> 
                <div key = {element.imdbID} className = "nominantes-item"> 
                    <div className="nominantes-img" style={{ backgroundImage: `url(${element.Poster})`}}></div> 
                    <p>{element.Title} - {element.Year}</p>
                    <button className = "nominantes-button"
                        onClick= {()=>this.props.removeNominantHandler(element.imdbID)}
                    >                    
